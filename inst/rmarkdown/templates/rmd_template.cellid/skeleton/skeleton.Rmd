...
**Preview pictures** with FIJI-ImageJ before you being, a simple Macro can be run to load virtual hyperstacks of any experiment:

```{Java}
// HyperStack para taguear
run("Image Sequence...", 
    "open=/path_to_your_r_libraries/4.2/rcell2.cellid/extdata/time_series_image_samples// file=(.*\\d.tif$) sort use");
run("Stack to Hyperstack...", 
    "order=xyczt(default) channels=73 slices=48 frames=3 display=Grayscale");
```

> Replace `path_to_your_r_libraries` and adjust the number of `channles`, `slices` and `frames` to your image set. 
> 
> Note: the dimensions names may be mismatched, because virtual stacks must be`xyczt` order (time is likely to end up as the "channels" dimension), but the order in which images are loaded depends on how their files are named (and thus ordered).

La única restricción es que solo podemos tener 3 "dimensiones extra": `frame`, `plane`, y `channel`. Esa limitación es molesta cuando tenemos todo eso, y además tenemos las fotos y los `.out` de cellid (para el que necesitariamos una _cuarta_ dimensión).

Otra aclaración: las fotos en el folder tendrían que estar ordenadas por "xyczt" para armar un virtual hyperstack. Ese casi nunca es el caso, entonces hacemos de cuenta, por ejemplo, que la dimensión "`c`" es el tiempo (porque las fotos se ordenan primero por t.frame), que "`z`" es la posición (porque las fotos se ordenan segundo por posicion), y que "`t`" es el canal (porque las fotos se ordenan último por channel).
...
data.dir <- system.file("extdata/time_series_image_samples/", package = "rcell2.cellid")

# Optional: use this if you have images from Metamorph's MDA, you may need to rename them
# rename_mda(images.path = images.path)

# Optional: Define a set of parameters for CellID.
...
cell2.out <- 
  rcell2.cellid::cell2(arguments = cellid.args, 
                       ...
                       # dry = T,  # To test stuff without overwritting previous output files.
...
Access the main output and plot:
...
Youmay want to check out the Shiny/Magick rcell2 analisys package:
...
Puede ser conveniente guardarlo en un archivo Rdata.

```{r}
save.image("resultado_segmentacion.Rdata")
```
...
```{r}
metamorph_pics_dir <- 
  system.file("extdata/time_series_image_samples/", 
...
```{r}
data.dir <- 
  system.file("extdata/time_series_image_samples/", package = "rcell2.cellid")
...
```{r}
# Get image paths
cellid.args <- rcell2.cellid::arguments(path = new.data.dir,
                                        file.pattern = "^(BF|[TRY]FP)_Position(\\d+)_time(\\d+).tif$") %>% 
...
File patterns may vary, depending on what images were provided as input, and the identifiers in their file names.

Fluorescent channel pattern examples for `cell.load.alt` and `parameter_scan`; choose one and comment the others:
...
cell.command.path = NULL # Use the built-in binary.
```
...
#### Review stacks in ImageJ

##### Auto mode

Make TIFF stacks for a particular channel, and prepare a copy-paste-ready ImageJ Macro, to load them as Hyperstacks.
...
```{r}
stack.paths <- 
  make_scan_stacks(results.bound = results.bound, test.dir = test.dir, stack.channels = "BF.out")
```
...
Installation instructions for `rcell2` and `rcell2.magick` packages can be found [here](https://github.com/darksideoftheshmoo/rcell2/tree/master#installing-the-package) and [here](https://github.com/darksideoftheshmoo/rcell2/tree/rcell2.magick#installing-the-package).
