...
```{r}
template_path <- get_spos_template()
```
...
> **Note**: the usage of the "`file.pattern`" argument is explained in the
> [Arguments] section of this notebook. The following will work with the example
> time-series images.
...
```{r}
stg_list_file <- "path/to/your/positions_list_file.STG"

stg_list <- stg_list_file |> 
  read.csv(skip = 4, header = F)

names(stg_list)[1:5] <- c(
    "position_name",
    "x", "y", "z",
    "af_offset"
  )
