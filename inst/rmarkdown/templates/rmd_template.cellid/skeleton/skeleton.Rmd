...
For many-well experiments in a 384 microscopy plate, you may benefit from generating stage coordinates for MetaMorph's MDA automatically.

First, open and fill out the template spreadsheet, which must contain sheets with these names:

1. "`pdata`": the [position metadata]. This table will be useful in the future too (e.g. for analyzing results).
2. "`well_order`": a representation of the plate indicating the order in which to take pictures.
3. "`well_images`": a representation of the plate indicating how many images to acquire in each well.
...
The `pdata` table holds metadata about each stage position, each identified by a unique integer index (e.g. 0, 1, 2, etc.).

You can generate your own `pdata` table and save it to a CSV file. It only requires a "`pos`" column for the position index. Additional columns can be added, containing metadata (e.g. experimental conditions, strain identifiers, etc). You can then load this table with R's `read.csv` function.

This package includes a `make_pdata` function, that will extract the `pdata` table from a spreadsheet template, like the one used above.

If you added the metadata to other sheets in "well-plate" layout, you can incorporate it to the full `pdata` table by setting the `pdata_sheets` argument of the `make_pdata` function. This argument should be a vector with the names of those sheets.
This can work in combination with the table in the `pdata` sheet of the spreadsheet.

```{r}
pdata <- make_pdata(template_path)

# Un-comment to extract additional metadata in "well-plate" layout from other sheets.
#
...
note that it has **3 capturing groups** (i.e. the _stuff_ between parentheses).
...
Sometimes time frames for a given channel are stored as a stack, instead of individual images.

To split a stack into individual images, and name them appropriately, you can use R's tools.

```{r}
stack_files <- dir(data.dir, pattern = ".tif", full.names = T)

stack_files[1]
```

```{r}
paths <- data.frame(pic.path = data.dir,
                    pic.file = basename(stack_files)) %>% 
  extract(pic.file, c("pos", "ch"), regex = "Stack_Position(\\d+)_([A-Za-z]+)\\.tif$", remove = F) %>% 
...
```{r}
split_dir <- file.path(data.dir, "split")
dir.create(split_dir, showWarnings = F)

split_dir
```

We will need the ijtiff package to split the stacks:
...
Extract the frames from the selected stacks.

```{r}
pad_width <- paths$pos |> 
  as.character() |> nchar() |> max()

selected_paths |> 
  apply(MARGIN = 1, FUN = function(d){
    # Load one image.
    pic <- file.path(d["pic.path"], d["pic.file"]) |> ijtiff::read_tif(msg = F)
    pic.n_stacks <- dim(pic)[4]
    frame_pad_width <- pic.n_stacks |> as.character() |> nchar()
    ...
    for(i in 1:pic.n_stacks){
      new_filename <- file.path(
        ...
        paste0(
          ...
          # Position index.
          "_Position",
          str_pad(string=d["pos"], width=pad_width, pad = "0"),
          # Time index.
          "_time",
          str_pad(string=i, width=frame_pad_width, pad = "0"),
          # Extension.
          ".tif"
        )
      )
      ...
      pic_data <- pic[,,,i]
      ijtiff::tif_write(pic_data, msg = F,
                        path = new_filename,
                        overwrite = T)
    }
  })
...
Now adjust your `data.dir` to get images from the new "split" sub-directory:
...
If your experiment only contains bright-field images (a.k.a. "BF") you must
set `bf_as_fl = T`. The function will otherwise complain about missing
...
files in the `data.dir` folder, and the segmentation parameters.

The `arguments` function accepts segmentation parameters as a list, as a path to
a [parameters file], or a `data.frame` with position numbers and paths to 
parameter files. Run `?arguments` for details
...
```{r}
# Default value for the common channels (no z-stacks):
arguments.file.pattern <- "^(BF|[A-Z]FP)_Position(\\d+)_time(\\d+).tif$"

# Pattern for a time-course with z-stacks as channels:
# arguments.file.pattern <- "^(BF|[A-Z]\\d+)_Position(\\d+)_time(\\d+).tif$"

# Pattern for BF or TFP only, no t.frame info.
...
0. Relax segmentation parameters:
  - Set `min_pixels_per_cell` to a low value (e.g. `30`) and `max_pixels_per_cell` to a high value (e.g. `5000`).
  - Set `max_dist_over_waist` (split) and `max_split_over_minor` (rejoin) to high values (e.g. `80` and `5`).
1. Now scan over `background_reject_factor` *only* (e.g. between `0` and `2`).
...
3. Scan over `max_dist_over_waist`.
  - At this step you must not consider projected growth (i.e. shmoos) being split 
    from the rest of the cell body, because `max_split_over_minor` is essentially disabled.
  - Set `max_dist_over_waist` to its optimal value, which adjacent cells and 
    daughters cells from their mothers.
5. Scan over `max_split_over_minor` *only*, which should minimize the amount of 
   growth projections split from their bodies, while allowing buds to remain
   split from their mothers.
...
parameters.list <- rcell2.cellid::parameters_default(
  # Extreme values to detect all.
  max_pixels_per_cell = 5000, min_pixels_per_cell = 30,
  
  # Large values to effectively disable these features temporarily.
  max_dist_over_waist = 80, max_split_over_minor = 5,
  
  # Scan over bgrf first.
  background_reject_factor = seq(from=0.2, to=1.4, length.out=6)
  
  # Then scan over max_dist_over_waist, and then max_split_over_minor,
  # as explained above.
)
...
Both `rcell2.cellid` and `rcell2` provide the `get_cell_boundaries` function, which can 
...
To load the boundary pixels, use the `get_cell_boundaries` function. It can load this 
...
cell.boundaries <- 
  rcell2.cellid::get_cell_boundaries(
    data = cellid.args,
...
This path can be passed to the `arguments()` function in the [Arguments] section
of this notebook.
