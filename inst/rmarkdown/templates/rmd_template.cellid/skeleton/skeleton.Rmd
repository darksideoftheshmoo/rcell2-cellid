...
### Check that yor images are there
...
Set details from your microscope: magnification and physical size of the camera's pixels.
...
- **Sequential** run: one image is processed at a time, which can be slow on large data sets.
...
fft.bfs.dir <- paste0(data.dir, "/", fft.bfs.subdirectory)
dir.create(fft.bfs.dir)

# Link them over
result <- file.symlink(from = normalizePath(bf.files),
...
if(length(all.files) == length(bf.files) + length(the.rest)){
  file.symlink(from = normalizePath(the.rest),
               to = paste0(dirname(the.rest[1]), "/", fft.bfs.subdirectory, "/filtered/", basename(the.rest)))
}
```
...
filtered.bfs.dir <- paste0(data.dir, "/fft_images_dataset/")
