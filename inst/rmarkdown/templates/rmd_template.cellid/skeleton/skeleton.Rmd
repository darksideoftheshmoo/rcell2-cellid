...
For full usage details, skip to the next main heading, named "Full usage guide", and read it carefully.

Always read the `?help` before using a function ;) specially when they do file operations, which are irreversible.
...
Customize parameters for CellID (details below).

```{r}
parameters.txt <- rcell2.cellid::parameters_write(rcell2.cellid::parameters_default())
```

Prepare the "arguments" dataframe for CellID, 
which lists the image files that match the regular expression specified in `file.pattern`:
...
cellid.args <- 
  rcell2.cellid::arguments(path = data.dir,
                           ...
                           # file.pattern = "^(BF|[A-Z]FP)_Position(\\d+)().tif$"  # Settings for a single time frame experiment.
...
Optional: run the following, line by line, to preview its features.
