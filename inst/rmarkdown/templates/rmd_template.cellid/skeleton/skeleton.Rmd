...
and a [Usage guide] with in-depth explanations and advanced usage examples.
...
Here is a valid, hand-made parameter list, with the defaults I like:
...
`parameters_default` is a helper function to prepare the parameters list more
easily, and also documents them (run `?parameters_default` for help).
...
The simplest way to generate parameter combinations is with rcell2's `parameters_default` and R's `expand.grid`.

Because scanning over all parameters space would take ages, you can follow a simplified procedure of 3 small scans, on a small subset of images. Here is a procedure to reduce the scan's complexity:

1. First scan over `background_reject_factor` *only*.
2. Fix `background_reject_factor` to its optimal value, which minimizes segmentation artifacts.
3. Scan over `max_dist_over_waist` **and** set `max_split_over_minor` to a high value (e.g. `max_split_over_minor=5`).
  - This is because `max_dist_over_waist` splits "figure-8" cells into two, and `max_split_over_minor` will re-join shmoo-like cells, thus confounding the interpretation of the parameter scan.
4. Fix `max_dist_over_waist` to its optimal value, which adyacent cells and daughters cells from their mothers.
  - At this point you must tolerate projected growth (i.e. shmoos) being split from the rest of the cell body.
5. Scan over `max_split_over_minor` *only*, which should minimize the amount of growth projections split from their bodies.

In the following example, only `background_reject_factor` is scanned, across values produced with R's `seq` function:
...
This function will make TIFF stacks, which can take a long time if the set is large.
...
indicative of segmentation quality, and may be useful when filtering 
programatically (i.e. without visual inspection).
...
* Derived measures:
  ...
  * `roughness` & `roughness2`: Inidicative of the average "roughness" in the evolution of `a.tot` across time frames.
