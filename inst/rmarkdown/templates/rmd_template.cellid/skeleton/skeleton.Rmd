...
```{r}
result <- rename_mda(images.path = data.dir,
                     # Set the rename function to NULL to disable renaming.
...
The above list contains all images, which differ only by a prefix in their file names.
...
rename.df <-
  ...
  mutate(exp.group = basename(file) |> sub(pattern="^focus(\\d+)_.*", replacement="\\1") |> as.integer()) |> 
