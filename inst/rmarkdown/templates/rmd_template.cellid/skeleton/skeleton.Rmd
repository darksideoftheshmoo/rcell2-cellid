...
```{r}
# Choose one set of example images:
...
Cell boundaries and interior pixels coordinates can be loaded with `cell.load.boundaries`,
also provided by the `rcell2` package.
...
Using Metamorph's _Multi-dimensional acquisition_ (MDA for short), may use file 
...
The `rename_mda` function renames those weird file names, making them compatible
with `rcell2`'s defaults.
...
Configure the Cell-ID parameters that affect cell segmentation, tracking, and some corrections.
...
But, here is a valid, hand-made parameter list, with the same defaults:
...
Sometimes we want to test out many parameter combinations fefor running Cell-ID in the full dataset.
...
scan.arguments <- cellid.args %>% 
  subset(pos %in% test.pos & t.frame %in% test.frames)
...
scan.results <- 
  parameter_scan(parameters.df = parameters.df,
                 scan.arguments = cellid.args.scan)
```
