...
Using Metamorph's _Multi-dimensional acquisition_ (MDA for short), may result in file names
different from the default style (i.e `YFP_Position01_time1.tif`, and so on).

The default pattern, described at the [Image file names] section of this notebook.
`rename_mda` will help you rename file names to match that pattern.

The function is provided by the `rcell2.cellid` package. A full example on how to use it is 
available below, in the [Renaming images] section of this notebook.
...
> in the [Images and Arguments] section.
...
cell.boundaries <- 
  rcell2.cellid::cell.load.boundaries(data.source = "masks.tsv",
...
> [Images and Arguments] section of this notebook.
...
Run the following code, copy the output, and paste it into a new "Macro" window in ImageJ/FIJI. Run the macro code in ImageJ, a new hyperstack will open.
...
Define the path to the images of your experiment.
...
time-course experiment. Remember to omit the corresponding lines when using your own
images!
