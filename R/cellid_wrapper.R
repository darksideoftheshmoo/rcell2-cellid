...
#' @param align_fl_to_bf Default: \code{F} Frame alignment. Cell-ID can perform image registrations, moving the the frame in XY to align it to a reference image. If “align FL to BF” is selected the bright field image is used as reference. If “align FL to first” is selected the first fluorescence image is used as reference. These options are especially useful when sampling different positions, as re-positioning of the microscope stage might introduce some displacement between consecutive images.
...
#' 
#' @param arguments The "arguments" data frame, as generated by \code{rcell2.cellid::arguments()}.
#' @param bf_as_fl Set this to TRUE if the input already contains BF images as channels (i.e. if \code{bf_as_fl} was set to TRUE when generating the arguments data frame).
#' 
#' @return A data.frame similar to \code{get_cell_data()$images}.
#' @import dplyr
#' @export
arguments_to_images <- function(arguments, bf_as_fl=F){
  ...
  # Skip adding the BF if requested.
  if(bf_as_fl){
    images_pre <- fl
  } else {
    images_pre <- dplyr::bind_rows(bf, fl)
  }
