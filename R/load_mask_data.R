...
#' 
#' @param data If \code{data.source = "masks.tsv"}, a "positions" data frame (i.e. a data.frame/list with one "output" column/item, containing paths pointing to the output directories generated by Cell-ID) or, equivalently, the "arguments" dataframe, as produced by \code{rcell2.cellid::arguments}. If \code{data.source = "out.tif"}, a cell.data object (prepared with \code{rcell2.cellid::cell.load.alt}).
...
#' @param flags Used to subset the input files by Cell-ID's "flag" field (requires awk). Each flag corresponds to an imaging channel, according to a "mapping" found in \code{cell.data$mapping} (available when using \code{cell.load.alt}).
