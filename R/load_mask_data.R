...
read_tiff_masks <- function(path, cell_id_offset = -1, interior_offset = NULL, blank_bg = NULL){
  ...
  if(!requireNamespace("ijtiff")){
    stop("read_tiff_masks: requires functions from the 'ijtiff' package, which is not installed.")
  }
...
#'
#' @param positions A "positions" dataframe (i.e. a data.frame/list with one "output" column/item, containing paths pointing to the output directories generated by Cell-ID). Alternatively, the "arguments" dataframe, as produced by \code{rcell2.cellid::arguments} for the experiment. Required only for \code{data_source = "masks.tsv"}.
...
#' @export
tsv_paths_from_args <- function(positions,
                                position_pattern = ".*Position(\\d+)$",
                                tsv_pattern = "^out_all_masks.tsv(?:\\.gz)?$"){
  
  tsv_paths <- dir(unique(positions$output), 
                   pattern = tsv_pattern, 
                   full.names = T)
  
  if(length(tsv_paths) != length(unique(positions$output))) 
    stop("tsv_paths_from_args error: the number of TSVs found does not match the positions length")
...
#' 
#' @param data If \code{data_source = "masks.tsv"}, a "positions" data frame (i.e. a data.frame/list with one "output" column/item, containing paths pointing to the output directories generated by Cell-ID) or, equivalently, the "arguments" dataframe, as produced by \code{rcell2.cellid::arguments}. If \code{data_source = "out.tif"}, a cell.data object (prepared with \code{rcell2.cellid::get_cell_data}).
#' @param data_source Either "out.tif" or "masks.tsv".
#' @param pixel.type When \code{data_source = "masks.tsv"}, you may choose the pixel "type". At least one of \code{c("i", "b")} for interior and/or boundary pixels ("b" by default).
#' @param flags Used to subset the input files by Cell-ID's "flag" field (requires awk). Each flag corresponds to an imaging channel, according to a "mapping" found in \code{cell.data$mapping} (available when using \code{get_cell_data}).
#' @param close.paths When TRUE and \code{data_source = "masks.tsv"}, append the first row to the end of the data.frame (groping by cellID and pos). Useful for plotting of making closed polygons.
#' @param tiff.channel When \code{data_source = "out.tif"}, provide the channel name for images holding boundary data ('BF.out' by default).
...
get_cell_boundaries <- function(data,
  ...
  option.check <- sum(data_source %in% c("out.tif", "masks.tsv"))
  if(option.check != 1) stop("'data_source' must be either 'out.tif' or 'masks.tsv' ")
  
  if(data_source == "out.tif"){
    cell.data <- data
  ...
  if(data_source == "masks.tsv"){
    positions <- data
    
    stopifnot(!is.null(positions))
    stopifnot(is.data.frame(positions))
    
    if(!is.null(flags))
      awk_term <- paste0(" | awk -F '\t' '{ if (NR==1 || ", 
                         paste0("$3 == ", flags, collapse = " || "),
                         ") { print } }'")
    else
      awk_term <- ""
    ...
    # Load (and filter) cell pixels.
    cell.boundaries <- 
      tsv_paths_from_args(positions=positions, 
                          position_pattern=position_pattern,
                          tsv_pattern=tsv_pattern) %>% 
      with(setNames(path, pos)) %>% 
      # Load pixels using fread and grep.
      lapply(function(file.name) data.table::fread(
        
        
        cmd = paste0(
          # Stream the compressed file to grep.
          "gunzip -c '", normalizePath(file.name), "'",
          # Keep the header (with a column name condition) and 
          # filter by pixel type before loading data into R.
          grep_term,
          # Filter by flag using awk.
          awk_term
        ),
      ...
      # Cell-ID XY positions are zero-indexed, add 1 and enter the R inferno:
      mutate(x=x+1,y=y+1)
    
    if(!is.null(pixel.type)){
      cell.boundaries <- cell.boundaries[cell.boundaries$pixtype %in% pixel.type, ]
    }
    
    if(!is.null(flags)){
      cell.boundaries <- cell.boundaries[cell.boundaries$flag %in% flags, ]
    }
...
mask_df_from_tiff <- function(tiff_path, image_bits, cell_id_offset = -1){
  ...
  if(!requireNamespace("tiff")){
    stop("mask_df_from_tiff: requires functions from the 'tiff' package, which is not installed.")
  }
