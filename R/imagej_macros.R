...
#' @param temp_dir_name Base-name for the temporary directory where renamed links to images are stored. It will be created as a subdirectory of \code{tempdir()}. Any directory in this path with the same name will be deleted before use.
...
#' @export
ijm_open_hyperstack <- function(images, use_out = 0:1, temp_dir_name="ijm_set", macro_file=TRUE, fix_order=TRUE, add_tools=TRUE){
  # Check arguments.
  ...
  if(fix_order){
    ...
    img.dir <- file.path(tempdir(check = T), temp_dir_name)
